...
class DocResultsTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        AF.request("https://prod.maneesh.me/v1/doctors/").responseDecodable(of: doctApi.self) { response in
            switch response.result{
                case .success(let docRes) :
                    print("Sucess")
                    self.doctors = docRes.docs
...
//        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath)
        ...
        cell.detailTextLabel?.text = doctors[indexPath.row].address
        return cell
    }
